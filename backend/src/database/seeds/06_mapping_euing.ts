...
const euingMapping = {
  ...
  frais_ttc: 'Frais TTC',
  commission:
    "(service.startsWith('PAIEMENT') ? Com_Cmp_Payeur : Com_Cmp_Envoyeur)",
...
export async function seed(knex: Knex): Promise<void> {
  // Vide la table 'mapping' avant d'insérer les nouvelles données
  await knex('mapping').del();

  // Insère le mapping pour l'état 'euing'
  await knex('mapping').insert({
    etat: 'euing',
    mapping: JSON.stringify(euingMapping),
  });

  console.log('Mapping EUING inséré avec succès');
}
